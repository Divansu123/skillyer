    ...
    >
      <button
        ...
        style={{
          ...
          background: page <= 1 ? "var(--bg2)" : "white",
      ...
      {items.map((p, i) =>
        p === "..." ? (
          <span
            key={"d" + i}
            style={{ padding: "5px 8px", color: "var(--text3)", fontSize: 13 }}
          >
            …
          </span>
        ) : (
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 7,
          border: "1px solid var(--border)",
          background: page >= pages ? "var(--bg2)" : "white",
...
const StatusBadge = ({ active }) => (
  <span className={`admin-badge ${active ? "green" : "orange"}`}>
...
const DashboardSection = ({ stats }) => {
  ...
  const actionColor = (a) =>
    a === "Enrolled" ? "blue" : a === "Applied for Job" ? "orange" : "green";
  ...
  return (
    <div className="admin-section active">
      ...
      <div className="admin-card">
        ...
        {loadingActivity ? (
          <div
            style={{ textAlign: "center", padding: 24, color: "var(--text3)" }}
          >
            Loading activity…
          </div>
        ) : activity.length === 0 ? (
            ...
            <tbody>
              {activity.map((a, i) => (
                <tr key={i}>
                  <td>{a.user}</td>
                  <td>
                    <span className={`admin-badge ${actionColor(a.action)}`}>
                      {a.action}
                    </span>
                  </td>
                  <td>{a.item}</td>
...
const CourseForm = ({ data, onChange, providers }) => (
  <>
    <div className="admin-form-row">
      ...
      <div>
        ...
        <select
          ...
          value={data.provId}
          onChange={(e) => onChange({ ...data, provId: e.target.value })}
        >
          {providers.map((p) => (
    ...
    <div className="admin-form-row">
      <div>
        ...
        <select
          className="admin-select"
          value={data.catId}
          onChange={(e) => onChange({ ...data, catId: e.target.value })}
        >
          {[
            "tech",
            "data",
    ...
    <div className="admin-form-row">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => onChange({ ...data, origPrice: e.target.value })}
        />
      </div>
    </div>
    <div className="admin-form-row">
    ...
    <div className="admin-form-row">
      <div>
        <label className="admin-form-label">Students Enrolled</label>
        <input
          className="admin-input"
          value={data.students}
          onChange={(e) => onChange({ ...data, students: e.target.value })}
          placeholder="e.g. 12k+"
        />
      </div>
      <div>
    ...
    <div className="admin-form-row">
      <div>
        <label className="admin-form-label">Certificate?</label>
        <select
          className="admin-select"
          value={String(data.hasCert)}
          onChange={(e) =>
            onChange({ ...data, hasCert: e.target.value === "true" })
          }
        >
      ...
      <div>
        <label className="admin-form-label">Exp Level</label>
        <select
          className="admin-select"
          value={data.expLevel}
          onChange={(e) => onChange({ ...data, expLevel: e.target.value })}
        >
          {["any", "fresher", "mid", "senior"].map((l) => (
            <option key={l}>{l}</option>
          ))}
        </select>
      </div>
    ...
    <div style={{ display: "flex", gap: 24, marginTop: 12, flexWrap: "wrap" }}>
      ...
      >
        <input
          ...
          onChange={(e) => onChange({ ...data, isImportant: e.target.checked })}
        />
...
const CoursesSection = ({ providers }) => {
  ...
  const blank = {
    title: "",
    provId: providers[0]?.id || "coursera",
    catId: "tech",
    level: "Beginner",
    duration: "",
    price: 0,
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast(e.response?.data?.message || "Error adding course", "error");
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      const { id, provider, category, createdAt, updatedAt, ...data } =
        editingCourse;
  ...
  const handleToggleActive = async (c) => {
    try {
      await updateCourse(c.id, { isActive: !c.isActive });
      load(page);
      toast(
        c.isActive ? "Course hidden from portal" : "Course is now live!",
        "success",
      );
    } catch {
      toast("Error", "error");
  ...
  const handleToggleImportant = async (c) => {
    try {
      await updateCourse(c.id, { isImportant: !c.isImportant });
      load(page);
      toast(
        c.isImportant
          ? "Removed from homepage"
          : "⭐ Added to homepage Top Courses!",
        "success",
      );
    } catch {
      toast("Error", "error");
    ...
    <div className="admin-section active">
      ...
      <div className="admin-header">
        <div className="admin-title">
          📚 Manage Courses{" "}
          <span
            style={{ fontSize: 13, fontWeight: 400, color: "var(--text3)" }}
          >
            ({total} total)
          </span>
        </div>
              ...
              <tr key={c.id} style={{ opacity: c.isActive ? 1 : 0.5 }}>
                ...
                <td>
                  <button
                    ...
                    title={
                      c.isImportant ? "Remove from homepage" : "Add to homepage"
                    }
                    style={{
                      background: "none",
                      border: "none",
...
const PartnersSection = ({ providers, onRefresh }) => {
  ...
  const handleEditSave = async () => {
    ...
    try {
      const { id, createdAt, updatedAt, coursesList, _count, ...data } =
        editingPartner;
          ...
          <div className="admin-form-row">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingPartner((p) => ({ ...p, name: e.target.value }))
                }
              />
            </div>
            <div>
              <label className="admin-form-label">Logo Text (2–3 chars)</label>
              <input
                className="admin-input"
                maxLength={3}
                value={editingPartner.logo}
                onChange={(e) =>
                  setEditingPartner((p) => ({ ...p, logo: e.target.value }))
                }
              />
            </div>
          </div>
          <div className="admin-form-row">
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  type="color"
                  value={editingPartner.color || "#6246ea"}
                  onChange={(e) =>
                    setEditingPartner((p) => ({ ...p, color: e.target.value }))
                  }
                  style={{
                    width: 44,
                    height: 38,
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  type="color"
                  value={editingPartner.bg || "#f0f2f8"}
                  onChange={(e) =>
                    setEditingPartner((p) => ({ ...p, bg: e.target.value }))
                  }
                  style={{
                    width: 44,
                    height: 38,
        ...
        <table className="admin-table">
          ...
          <tbody>
            {providers.map((p) => (
              <tr
                key={p.id}
                style={{ opacity: p.isActive !== false ? 1 : 0.5 }}
              >
                <td>
                  <div
...
const DomainsSection = ({ categories, onDelete, onRefresh }) => {
  ...
  const handleSave = async () => {
    if (!newDomain.name.trim()) {
      toast("Domain name required", "warning");
      return;
    }
    try {
  ...
  const handleEditSave = async () => {
    ...
    try {
      const { id, createdAt, updatedAt, count, courses, _count, ...data } =
        editingDomain;
  ...
  const handleDeleteConfirm = async (id) => {
    const ok = await confirm(
      "Delete this skill domain? This may affect courses in this category.",
    );
    if (!ok) return;
    onDelete(id);
  };
    ...
    <div className="admin-section active">
          ...
          <div className="admin-form-row">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewDomain((d) => ({ ...d, name: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewDomain((d) => ({ ...d, icon: e.target.value }))
                }
              />
            </div>
          </div>
      ...
      {editingDomain && (
        ...
        >
          ...
          <div className="admin-form-row">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingDomain((d) => ({ ...d, icon: e.target.value }))
                }
              />
            </div>
          </div>
          <div className="admin-form-row">
            <div>
              <label className="admin-form-label">Course Count (display)</label>
              <input
                className="admin-input"
                type="number"
                value={editingDomain.count || 0}
                onChange={(e) =>
                  setEditingDomain((d) => ({
                    ...d,
            ...
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              ...
              >
                <input
                  type="checkbox"
                  checked={editingDomain.isActive !== false}
                  onChange={(e) =>
                    setEditingDomain((d) => ({
                      ...d,
                      isActive: e.target.checked,
        ...
        <table className="admin-table">
          ...
          <tbody>
            {categories.map((c) => (
              <tr
                key={c.id}
                style={{ opacity: c.isActive !== false ? 1 : 0.5 }}
              >
                <td>{c.name}</td>
          ...
          <tbody>
            {filtered.map((e) => (
              <tr key={e.id}>
                ...
                <td style={{ color: "var(--text3)" }}>{e.location || "—"}</td>
                <td style={{ color: "var(--text3)" }}>{e.preference || "—"}</td>
                <td>
                  <span
                    style={{
                      fontSize: 10,
                ...
                <td>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      onClick={() =>
                        navigate(`/admin/edit-enrollment/${e.id}`, {
                          state: {
                            name: e.name,
                            email: e.email,
                            phone: e.phone,
...
const JobsSection = () => {
  ...
  const handleToggleActive = async (j) => {
    try {
      await updateJob(j.id, { isActive: !j.isActive });
      load(page);
      toast(
        j.isActive ? "Job hidden from portal" : "Job is now live!",
        "success",
      );
    } catch {
      toast("Error", "error");
  ...
  return (
    <div className="admin-section active">
      ...
      {editingJob && (
        <div
          className="admin-card"
          style={{ marginBottom: 20, borderLeft: "3px solid #f97316" }}
        >
          <div
            style={{
              fontFamily: "Syne,sans-serif",
          ...
          <div className="admin-form-row">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, title: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, company: e.target.value }))
                }
              />
            </div>
          </div>
          <div className="admin-form-row">
            <div>
              ...
              <input
                className="admin-input"
                value={editingJob.cat}
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, cat: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, jobType: e.target.value }))
                }
              >
                {[
                  "fulltime",
          ...
          <div className="admin-form-row">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, location: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingJob((j) => ({ ...j, salary: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <input
            ...
            onChange={(e) =>
              setEditingJob((j) => ({ ...j, tags: e.target.value }))
            }
          />
          <label
            style={{
          ...
          >
            <input
              ...
              onChange={(e) =>
                setEditingJob((j) => ({ ...j, isActive: e.target.checked }))
              }
            />
            Active (visible in portal)
          </label>
      ...
      <div className="admin-card">
        ...
        {jobs.length === 0 && (
          ...
          >
            No jobs yet. Click "+ Post New Job".
          </div>
        )}
        <Pagination page={page} pages={pages} onPage={load} />
      </div>
...
const ApplicationsSection = () => {
  ...
  const LIMIT = 15;
  const API_BASE =
    process.env.REACT_APP_SERVER_URL || "https://skillyer-api.onrender.com";
      ...
      <div className="admin-header">
        <div className="admin-title">
          ...
          <span
            style={{ fontSize: 13, fontWeight: 400, color: "var(--text3)" }}
          >
            ({total} total — submitted via course page)
          </span>
        </div>
        <input
          className="admin-input"
            ...
            {filtered.map((e) => (
              <tr key={e.id}>
                ...
                <td>
                  ...
                  >
                    {["Active", "In Progress", "Completed"].map((s) => (
                      <option key={s}>{s}</option>
                    ))}
                  </select>
                </td>
...
const CounselSection = () => {
  ...
  const handleStatusChange = async (id, value) => {
    try {
      ...
      setCounsels((prev) =>
        prev.map((x) => (x.id === id ? { ...x, status: value } : x)),
      );
      ...
      <div className="admin-content">
        ...
        {section === "partners" && (
          <PartnersSection
            providers={providers}
            onRefresh={() =>
              fetchProviders().then((r) => setProviders(r.data.data))
            }
          />
        )}
        {section === "domains" && (
          <DomainsSection
            ...
            onRefresh={() =>
              fetchCategories().then((r) => setCategories(r.data.data))
            }
          />
        )}
